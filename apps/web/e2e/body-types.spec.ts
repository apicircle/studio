...
test.describe('Request body types', () => {
  test('none: wire body.kind === empty', async ({ app, e2eMock, sidebar }) => {
  ...
  test('json: wire body.kind === json with parsed shape', async ({
    app,
    monaco,
    e2eMock,
    sidebar,
  ...
  test('text: wire body.kind === text with raw content', async ({
    app,
    monaco,
    e2eMock,
    sidebar,
  ...
  test('xml: wire body.kind === text', async ({ app, monaco, e2eMock, sidebar }) => {
  ...
  test('urlencoded: wire body.kind === form with parsed key/value pairs', async ({
    app,
    monaco,
    e2eMock,
    sidebar,
  }) => {
    const path = '/anything/body-urlencoded';
    ...
    // The urlencoded body editor uses Monaco — one key=value per line.
    ...
    if (wire.body.kind === 'form') {
      expect(wire.body.form.name).toBe('alice');
      expect(wire.body.form.role).toBe('admin');
    }
  });

  test('form-data: text fields echo as multipart parts', async ({ app, e2eMock, sidebar }) => {
  ...
  test('form-data: file upload attaches and the wire receives a multipart file part', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('binary: file upload sends as application/octet-stream-like body', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('graphql: wire body wraps query+variables in JSON envelope', async ({
    app,
    monaco,
    e2eMock,
    sidebar,
