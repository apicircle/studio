...
test.describe('Push to save (P4.3a)', () => {
  test('happy-path: push commits workspace.json and renders the new SHA', async ({ app }) => {
    ...
    // Wire up the 5 push-flow endpoints. The first 4 are exact paths;
    // updateRef is PATCH on a path we already routed for createBranch
    // (POST), so use a method-aware handler.
    await fulfillJson(
      app,
    ...
    // Override the heads/<branch> route for PATCH (updateRef). Playwright
    // matches by URL, so we need a method-aware handler that defers GET
