...
test.describe('Web browser-specific', () => {
  ...
  test(tc(id('Vite Proxy'), '/_mock proxy serves same-origin'), async ({ app, e2eMock }) => {
    // The Vite dev server proxies /_mock/* to apps/e2e-mock. Hitting
  ...
  // TC-WB-0009 in apps/web/e2e/manual-residue.ts).
  // ---------------------------------------------------------------
  test(
  ...
  // exercise. They're listed in apps/web/e2e/manual-residue.ts and shown
