...
test.describe('Plan features — C10', () => {
  test('disabled step is skipped at run time (wire receives only the enabled steps)', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('stopOnAssertionFailure halts the loop after the first failed step', async ({
    app,
    e2eMock,
    mockApi,
    sidebar,
  ...
  test('duplicatePlan clones with "(copy)" suffix; both runnable independently', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('plan-level variables override env values during the run', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('plan-runs buffer caps at 200 — oldest evicts when a 201st run lands', async ({ app }) => {
  ...
  test('extractor with missing path → step 2 receives literal {{var}} (no fallback substitution)', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
    ...
    await app.evaluate(() => {
      ...
      w.__apicircleStore!.getState().setRequestExtractions(id, [
        ...
        { id: 'x-missing', variable: 'userId', source: 'body', path: 'missingKey', enabled: true },
      ]);
    });
  ...
  test('plan with duration assertion (lt 5000) on /delay/100 step passes', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('concurrent run on the same plan throws "plan already running"', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
