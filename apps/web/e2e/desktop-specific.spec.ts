// AUTO-GENERATED scaffold for module DS. Replace `test.fixme()`
// stubs with real assertions case-by-case in follow-up sessions.
//
// SUPERSEDED by apps/desktop/e2e/desktop-specific.spec.ts (S7). Real
// DS coverage now runs in the desktop Playwright project via the
// `_electron` harness. This file remains only so the lenient coverage
...
// each TC-DS cell to this spec. Cells with dedicated assertions
...
test.describe('TC-DS workbook iteration', () => {
  for (const [key, tcId] of Object.entries(tcMapDS)) {
