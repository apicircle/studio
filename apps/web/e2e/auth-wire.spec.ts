...
test.describe('Auth wire-level — programmatic types', () => {
  test('none: no Authorization header on the wire', async ({ app, e2eMock, sidebar }) => {
  ...
  test('bearer: token reaches the wire as `Authorization: Bearer <token>`', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
    ...
    await app.getByRole('textbox', { name: 'Bearer token', exact: true }).fill('e2e-bearer-token');
    await app.getByRole('button', { name: /^Send$/ }).click();
    await expect(app.getByText('200').first()).toBeVisible();
    const wire = await e2eMock.findLastByPath((p) => p === path);
  ...
  test('basic: encodes user:pass into Basic auth header', async ({ app, e2eMock, sidebar }) => {
  ...
  test('api-key (header): user-named header reaches the wire', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('api-key (query): user-named query param reaches the wire', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('api-key (cookie): user-named cookie composes (web-stripped, asserted via preview)', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('custom-header: user-named header reaches the wire', async ({ app, e2eMock, sidebar }) => {
  ...
  // and the mock-server side is covered by apps/e2e-mock/src/server.test.ts.
  // Desktop (native HTTP) does support NTLM round-trip end-to-end.
  test.skip('ntlm: Type-1 → Type-2 challenge → Type-3 → 200', async ({ app, e2eMock, sidebar }) => {
  ...
  test('hawk: HMAC-signed Authorization header verifies on the server', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('aws-sigv4: signed Authorization header verifies on the server', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
    ...
    await app.getByRole('textbox', { name: 'AWS access key ID', exact: true }).fill('AKIDEXAMPLE');
    await app
  ...
  // apps/e2e-mock/src/server.test.ts against the same server endpoint.
  test.skip('digest: full RFC 7616 challenge → response → 200', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('jwt-bearer: HS256-signed token reaches the wire and verifies on the server', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
...
test.describe('Folder auth — single send + plan run', () => {
  test('inherit (default): folder api-key reaches the wire on a freshly-created request', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
  ...
  test('bypass cue: setting request auth to "none" surfaces the warning + can flip to inherit', async ({
    app,
    sidebar,
  }) => {
  ...
  test('plan run: folder auth resolves the same way as a single send', async ({
    app,
    e2eMock,
    sidebar,
  }) => {
