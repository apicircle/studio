...
> APICircle Studio is in **Early Access**. The desktop binaries are not yet
> code-signed with an Apple Developer ID or a Windows EV certificate, so the
> first launch (and each auto-update) will trigger an OS security warning.
> This page walks through the one-time "allow" step on each platform.
