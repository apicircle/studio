...
APICircle ships a local mock server engine that turns OpenAPI / Swagger / Postman / Insomnia files into a running HTTP server on `localhost`. Definitions are workspace-scoped (push to git so teammates share them); runtime state is per-host.
