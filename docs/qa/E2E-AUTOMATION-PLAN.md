...
- [x] [`apps/web/e2e/fixtures/idbSeed.ts`](../../apps/web/e2e/fixtures/idbSeed.ts)
      ...
      fixtures under `e2e/qa/runner/fixtures/workspaces/` were
...
| 2026-05-15 (post-S2 + S3 partial)                   |              1903 |              50.5% | S1, S2 (partial), S3 (partial), S7, S8, S9, S10, S11, S12 | **S2 done**: WorkspaceSwitcher / Settings / Marketplace already had stable aria-labels — no UI changes needed. `workspace-management.spec.ts` Create/Switcher/Delete cells converted to live (12 pass, 21 fixme'd on infra blockers). `search-marketplace.spec.ts` 2/3 live. **S3 partial**: `apps/web/e2e/fixtures/idbSeed.ts` + `seededApp.ts` built and wired with 4 variants (empty / seeded / with-secrets / large-1k). **Blocker**: `e2e/qa/runner/fixtures/workspaces/*.json` shape diverges from the IDB-stored `WorkspaceSynced` — dumping a real running app's IDB and regenerating the fixtures unblocks the WS hydrate (3) + CR reference-safety (25) cells. `FULL_MM_SWEEP=1` runs 56 cells, 29 pass; the 27 failures are documented browser residue (HEAD+body, OPTIONS+body strip per WHATWG Fetch §3.1.5) plus urlencoded-KeyValueRows label drift. `tcMapMM` key-format changed in parallel from `\|` to `+` — `lookupTcId` updated to match.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
