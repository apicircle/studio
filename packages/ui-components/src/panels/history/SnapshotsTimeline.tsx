...
function SnapshotRow({
  ...
  return (
    <li className="flex items-center gap-2 rounded-sm border border-border bg-surface px-2 py-1.5 text-[0.6875rem]">
      ...
      <button
        type="button"
        onClick={() => downloadSnapshot(entry)}
        aria-label={`Download snapshot from ${formatTimestamp(entry.createdAt)} as JSON`}
        title="Download as JSON"
        className="inline-flex h-6 items-center gap-1 rounded-sm border border-border bg-card px-2 text-[0.625rem] text-text-muted hover:border-accent hover:text-accent"
      >
...
/**
 * Serialize a snapshot's captured synced document to a JSON file and hand
 * it to the browser as a download. The captured doc is already a complete,
 * self-contained `WorkspaceSynced` value, so this needs no extra assembly.
 */
function downloadSnapshot(entry: WorkspaceSnapshot): void {
  const json = JSON.stringify(entry.workspaceSyncedSnapshot, null, 2);
  const blob = new Blob([json], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const stamp = entry.createdAt.replace(/[:.]/g, '-');
  const anchor = document.createElement('a');
  anchor.href = url;
  anchor.download = `apicircle-snapshot-${stamp}.json`;
