...
//   - API Circle exchange (placeholder — will land when export ships)
//
...
function detect(text: string, format: SourceFormat): DetectedKind {
  ...
  if (format === 'apicircle' || isApicircleExchange(json)) {
    // Placeholder: API Circle's own export format is on the roadmap. We
    // recognize the marker so users can paste the file and see a clear
    // "not yet" message rather than a parser error.
    return {
      kind: 'apicircle',
      message:
        'API Circle exchange format is recognized but not yet importable. Import the source workspace via Link Workspace, or export to Postman v2.1 from the source and re-paste.',
    };
  }
...
function labelForDetection(d: DetectedKind): string {
  ...
  return 'API Circle exchange (placeholder)';
}
