// Auth editor for a single request. Mirrors v1's AuthPanel — all 15
// schemes are surfaced as a top-level dropdown; the type-specific form
// lives directly under it. Form state is the request's `auth` field;
// every change goes through `setRequestAuth` and is persisted with the
// rest of the synced doc.
...
const AUTH_GROUPS: Array<{ label: string; types: Array<{ id: RequestAuthType; label: string }> }> =
  [
    {
      label: 'Basic',
      types: [
...
export function AuthTab({ request }: AuthTabProps) {
  ...
  const onChangeType = (next: RequestAuthType) => {
    ...
    setRequestAuth(request.id, defaultAuthFor(next));
  };

  const update = <T extends RequestAuth>(patch: Partial<T>) => {
    setRequestAuth(request.id, { ...(auth as T), ...patch });
  };

  return (
    <div className="flex flex-col gap-3">
      <div className={fieldsetClass}>
        <label htmlFor={`auth-type-${request.id}`} className={labelClass}>
          Auth type
        </label>
        <select
          id={`auth-type-${request.id}`}
          value={auth.type}
      ...
      {auth.type === 'none' && (
        <p className={noteClass}>No authentication will be added to this request.</p>
      )}

      {auth.type === 'inherit' && (
        <p className={noteClass}>
          Auth will be inherited from the parent folder. Folder-level auth is configured on the
          folder itself (coming in P20).
        </p>
      )}

      {auth.type === 'bearer' && (
        <Field id={`bearer-${request.id}`} label="Token">
          <SecretInput
            id={`bearer-${request.id}`}
      ...
      {auth.type === 'basic' && (
        <div className={gridClass}>
          <Field id={`basic-user-${request.id}`} label="Username">
            <input
              id={`basic-user-${request.id}`}
              aria-label="Username"
              value={auth.username}
      ...
      {auth.type === 'api-key' && (
        <div className="flex flex-col gap-3">
          <div className={gridClass}>
            <Field id={`apikey-key-${request.id}`} label="Key">
              <input
                id={`apikey-key-${request.id}`}
      ...
      {auth.type === 'custom-header' && (
        <div className={gridClass}>
          <Field id={`hdr-key-${request.id}`} label="Header name">
            <input
              id={`hdr-key-${request.id}`}
          ...
          extra={
            <Field label="Client auth method">
              <select
                ...
                onChange={(e) => update({ clientAuthMethod: e.target.value as 'header' | 'body' })}
                className={cn(inputClass, 'max-w-xs')}
              >
    ...
    <div className="flex flex-col gap-3">
      <div className={gridClass}>
        {fields.map((field) => {
          ...
          const id = `oauth2-${requestId}-${String(field)}`;
