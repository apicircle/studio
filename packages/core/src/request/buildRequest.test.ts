...
describe('buildRequest', () => {
  ...
  it('composes all four pieces from a Request and auto-feeds API Circle headers', async () => {
  ...
  it('user-set X-Client-Version wins over the auto-fed header (case-insensitive)', async () => {
    ...
    expect(built.headers['X-Client-Name']).toBe('API Circle Studio');
  ...
  it('defaults regenerate trace ids per send when no overrides are supplied', async () => {
    ...
    expect(built.headers['X-Client-Name']).toBe('API Circle Studio');
    expect(built.headers['X-Client-Version']).toBe('1.0.0');
  });
