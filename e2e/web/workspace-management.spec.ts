...
test.describe('Workspace management — git', () => {
  ...
  gitTest(
    ...
    async ({ appWithGithubMock, mockGithub }) => {
      ...
      try {
        ...
        // server. Restore it — in `finally`, so it runs even on a thrown
        // assertion — so a later run reusing this token isn't pre-broken by
        // the downgrade.
  ...
  for (const key of NEEDS_RICHER_MOCK) {
    gitTest.fixme(tc(id(key), key), async () => {
      // Needs control-plane endpoints on e2e/mock /__gh to flip
