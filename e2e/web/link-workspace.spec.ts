...
test.describe('Link Workspace (P5.2)', () => {
  ...
  test(
    tc(
      ...
      'marketplace search appends `topic:apicircle` to the GitHub query (session-bound)',
    ),
    async ({ app }) => {
      await setupSession(app);
      ...
      // URL-encoded form of `payments topic:apicircle`.
      expect(capturedUrls[0]).toContain('q=payments%20topic%3Aapicircle');
