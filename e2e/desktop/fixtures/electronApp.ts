...
export async function launchElectron(opts: LaunchOpts = {}): Promise<{
  ...
  // spotlight overlay that blocks every click on the app behind it. The
  // web suite seeds this via addInitScript, but Electron has already
  // navigated by the time we hold the window handle, so seed + reload.
