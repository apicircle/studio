"""Build APICircle Studio Manual Test Cases workbooks (web + desktop) with
...
def add_request_editor_tests():
    ...
    t(WEB, "RE", "Tab Title", "UX/UI", "Browser tab title reflects workspace",
      ...
      "Tab title updates 'Production · APICircle'.", "Low")
    # Desktop-specific
    ...
    for fmt_label, fmt in formats:
        for shape_label, shape in request_shapes:
            ...
            steps = (
                ...
                f"3. Import into APICircle.\n"
                f"4. Compare resulting request to source.\n"
                f"5. (Optional) Export from APICircle as same format; diff."
            )
            expected = (
...
PLATFORM_BLURBS = {
    "web": ("Covers the APICircle Studio web build (apps/web). OAuth2 uses popup + "
    ...
    "desktop": ("Covers the APICircle Studio desktop build (Electron). OAuth2 uses a "
