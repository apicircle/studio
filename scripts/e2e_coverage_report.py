...
Scans `apps/web/e2e/**/*.spec.ts` (and `apps/desktop/e2e/**/*.spec.ts`
once it exists) for TC-ID coverage and cross-references against the
...
RESIDUE_FILE = REPO / "apps" / "web" / "e2e" / "manual-residue.ts"
...
def load_manual_residue(path: Path) -> dict[str, str]:
    """Parse apps/web/e2e/manual-residue.ts → map TC-ID → rationale.
...
def load_tc_maps(root: Path) -> dict[str, set[str]]:
    ...
    fixtures_dirs = [
        root / "apps" / "web" / "e2e" / "fixtures",
        root / "apps" / "desktop" / "e2e" / "fixtures",
    ]
...
def load_tc_map_lookups(root: Path) -> dict[str, dict[str, str]]:
    ...
    entry_re = re.compile(
        r'^\s*"((?:\\.|[^"\\])*)":\s*"(TC-[A-Z0-9]{2,3}-\d{4})"', re.MULTILINE
    )
    ...
    for fixtures in fixtures_dirs:
        ...
        for f in fixtures.glob("tcMap*.ts"):
            ...
            for m in entry_re.finditer(text):
                # JSON-style decode the key (handles escaped quotes etc.).
...
def render_markdown(
    ...
    *,
    ...
    lines.append(
        "- **Manual-residue tier** (`apps/web/e2e/manual-residue.ts`): TC-IDs "
